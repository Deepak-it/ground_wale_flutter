...
class _BookingsTurfScreenState extends State<BookingsTurfScreen> {
  ...
  void initState() {
    super.initState();
    final String incoming = widget.initialTab.trim().toLowerCase();
    _activeTab = _tabs.contains(incoming) ? incoming : 'upcoming';
    _future = _load();
  }
  ...
  Future<Map<String, dynamic>> _load() async {
    ...
    if (!session.hasGround && session.isAuthenticated) {
      session.setGroundId(
        await GroundWaleApi.instance.ensureGroundIdForOwner(session.ownerId!),
      );
    }
    ...
    final Future<Map<String, dynamic>> summaryFuture = GroundWaleApi.instance
        .getBookingSummary(session.groundId!, status: status);
    final Future<List<Map<String, dynamic>>> listFuture = GroundWaleApi.instance
        .listBookings(session.groundId!, status: status);
            ...
            children: <Widget>[
              ...
              ...bookings.map((Map<String, dynamic> item) {
                final String bookingStatus =
                    item['bookingStatus']?.toString() ?? 'pending';
                final String paymentStatus =
                    item['paymentStatus']?.toString() ?? 'pending';
                final String paymentMethod =
                    item['paymentMethod']?.toString().toUpperCase() ?? 'UPI';
                ...
                return TurfCard(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        'Captain: ${item['captainName'] ?? item['teamName'] ?? '-'}',
                        style: const TextStyle(color: Colors.white70),
                      ),
                      ...
                      Row(
                        ...
                        children: <Widget>[
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: _chipBg(bookingStatus),
                              borderRadius: BorderRadius.circular(6),
                            ),
                            child: Text(
                              _statusText(bookingStatus),
                              style: TextStyle(
                                color: _chipText(bookingStatus),
                                fontWeight: FontWeight.w600,
                              ),
                            ),
                          ),
                                ...
                                onPressed: () async {
                                  await Navigator.of(context).push(
                                    MaterialPageRoute<void>(
                                      builder: (_) => BookingDetailsTurfScreen(
                                        bookingId:
                                            item['_id']?.toString() ?? '',
                                      ),
                                    ),
